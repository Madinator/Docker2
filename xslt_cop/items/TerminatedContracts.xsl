<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="TerminatedContracts">

        <p class="title contract_group" style="padding-left: 10px;">
            <xsl:value-of select="TerminatedContracts/@stitle"/>
        </p>

        <xsl:if test="not(TerminatedContracts/Contract)">
            <p><xsl:value-of select="Footer/NoTerminatedContracts/@stitle"/></p>
        </xsl:if>



        <xsl:if test="TerminatedContracts/Contract">
            <xsl:for-each select="TerminatedContracts/Contract">
